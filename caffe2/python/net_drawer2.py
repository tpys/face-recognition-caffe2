...
def choose_color_by_layertype(layertype):
    ...
    """
    color = '#0F9D58'  # Default
    if layertype == 'Conv' or layertype == 'ConvTranspose':
        color = '#FF5050'
    elif layertype =='Relu' or layertype == 'PRelu' or layertype == 'LeakyRelu':
        color = '#0F9D58'
    elif layertype == 'AveragePool' or layertype == 'MaxPool':
        color = '#FF9900'
    elif layertype == 'FC' or layertype == 'MFC':
        color = '#CC33FF'
    return color
...
def get_layer_label(layer, rankdir):
    ...
    if rankdir in ('LR', 'RL'):
        ...
        separator = '\\n'
    if layer.type == 'Conv' or layer.type == 'ConvTranspose' or layer.type == 'AveragePool' or  layer.type == 'MaxPool':
        # Outer double quotes needed or else colon characters don't parse
        # properly
        kernel = [_arg_val(a) for a in layer.arg if a.name == 'kernel'][0]
        stride = [_arg_val(a) for a in layer.arg if a.name == 'stride'][0]
        pad = [_arg_val(a) for a in layer.arg if a.name == 'pad'][0]
        ...
        node_label = '"%s%s(%s)"' % (name, separator, layer.type)
    return node_label



def GetOpNodeProducer(append_output, rankdir, **kwargs):
    def ReallyGetOpNode(op, op_id):
        OP_STYLE['fillcolor'] = choose_color_by_layertype(op.type)
        node_name = get_layer_label(op, rankdir)
...
def GetPydotGraph(
    ...
    if node_producer is None:
        node_producer = GetOpNodeProducer(False, rankdir, **OP_STYLE)
